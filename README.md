...
9. **List Members Who Registered in the Last 3 years**:
```sql
SELECT member_name FROM members
WHERE reg_date >= CURRENT_DATE - INTERVAL'3 years';
```
...
```sql
SELECT ist.issued_id,ist.issued_book_name,ist.issued_date FROM issued_status AS ist
...
```sql
SELECT e.emp_name, b.*, COUNT(ist.issued_id) AS num_issued FROM employee AS e
JOIN issued_status as ist
ON e.emp_id = ist.issued_emp_id
JOIN branch b 
ON e.branch_id = b.branch_id
